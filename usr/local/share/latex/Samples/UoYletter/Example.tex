\documentclass[12pt,polychrome,thesuoy]{UoYletter}

\signature{Hes Lington-Hall}
\namequal{Prof. H. Lington-Hall, CDM, DDT}
\jobtitle{Layabout}
\extension{8888}
\loginid{hlh}
\ComputerScience
\emaildomain{devnull.york.ac.uk}        % must follow ``\ComputerScience''
\website{http://www.java.applets.R.us/}

\definecolor{brown}{rgb}{0.7,0.5,0.3}
\headcolour{brown}

\date{31 February 1944}

\begin{document}
\begin{letter}
        {Dr. A. Cademic\\The House\\Avenue Road\\The City\\CI9~~0OO}

\ourref{Sp/1/H-LH/GS}
\yourref{23 7-A/b}
\opening{Dear Arbuthnot,}

\re{Arachnophilia}

Thank you for your letter of 5th \emph{inst}.  The Tarantulas finished
off Agatha.  I hope you enjoy the enclosed; perhaps it could help with
the Problem Of The One Undergraduate Too Many!

\closing{Lurv `n' kisses,}

\encl{A Black Widow}
\ps{PS The Bird Eating Spiders don't.}

\end{letter}

\end{document}
